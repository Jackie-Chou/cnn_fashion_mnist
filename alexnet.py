...
class AlexNet(object):
  ...
  def create(self):
    ...
    conv1 = conv(self.X, 3, 3, 32, 1, 1, padding = 'VALID', name = 'conv1')
    pool1 = max_pool(conv1, 2, 2, 2, 2, padding = 'VALID', name = 'pool1')
    
    # 2nd Layer: Conv (w ReLu) -> Pool
    conv2 = conv(pool1, 3, 3, 64, 2, 2, padding = 'VALID', name = 'conv2')
    pool2 = max_pool(conv2, 2, 2, 2, 2, padding = 'VALID', name ='pool2')
    
    # 3th Layer: Flatten -> FC (w ReLu) -> Dropout
    flattened = tf.reshape(pool2, [-1, 3*3*64])
    fc3 = fc(flattened, 3*3*64, 1024, relu = False, name='fc3')
    dropout3 = dropout(fc3, self.KEEP_PROB)
    
    # 4th Layer: FC and return unscaled activations (for tf.nn.softmax_cross_entropy_with_logits)
    self.fc4 = fc(dropout3, 1024, self.NUM_CLASSES, relu = False, name='fc4')
...
def conv(x, filter_height, filter_width, num_filters, stride_y, stride_x, name,
  ...
  with tf.variable_scope(name) as scope:
    ...
    print "bias.shape: {}, conv.shape: {}".format(bias.shape, conv.shape)
